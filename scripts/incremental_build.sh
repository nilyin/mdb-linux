#!/bin/bash
# Docker incremental build with cache - optimized approach
...
docker run --rm \
  ...
  bash -c "
    ...
    make mdv_platform mdv_types mdv_net mdv_api mdv_storage mdv_crypto -j4
...
echo
echo "To run tests: ./test_data_consistency.sh"
